...
<div class="col-sm-9 col-md-10 mt-5 text-center">
    ...
    if ($result->num_rows > 0) {
        ...
        echo '<th scope="col">Origional Cost</th>';
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<td>' . $row['pOrgCost'] . '</td>';
            echo '<td>' . $row['pSellCost'] . '</td>';
            
            echo '<td>';
            echo '<form action="editproduct.php" method="POST" class="d-inline">';
            echo '<input type="hidden" name="id" value="' . $row['pid'] . '">'; // Enclosed in quotes
            ...
            echo '<input type="hidden" name="id" value="' . $row['pid'] . '">'; // Enclosed in quotes
            echo '<button type="submit" class="btn btn-warning mr-3 ms-2" name="issue" value="Issue">';
...
<div class="d-flex justify-content-end align-items-end position-fixed bottom-0 end-0 p-3">
    <a class="btn btn-danger" href="addproduct.php">
